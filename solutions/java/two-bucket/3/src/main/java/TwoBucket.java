...
class TwoBucket {
    ...
    TwoBucket(int bucketOneCap, int bucketTwoCap, int desiredLiters, String startBucket) {
        ...
        int invalidB1 = startBucket == "one" ? 0 : bucketOneCap;
        int invalidB2 = startBucket == "one" ? bucketTwoCap : 0;
        ...
        while (dequeStates.size() > 0) {
            ...
            if ((invalidB1 == state.b1 && invalidB2 == state.b2) || visited.contains(v)) {
                continue;
            }
            visited.add(v);

            dequeStates.add(new TwoBucketState(
                    Math.min(state.b1 + state.b2, bucketOneCap),
                    state.b2 - (Math.min(state.b1 + state.b2, bucketOneCap) - state.b1), 
                    state.step + 1
                ));
            dequeStates.add(new TwoBucketState(
                    state.b1 - (Math.min(state.b1 + state.b2, bucketTwoCap) - state.b2),
                    Math.min(state.b1 + state.b2, bucketTwoCap),
                    state.step + 1
                ));
    ...
    class TwoBucketState {
        ...
        public boolean equals(Object o) {
            ...
            TwoBucketState tbs = (TwoBucketState)o;
            return tbs.b1 == this.b1 && tbs.b2 == this.b2 && tbs.step == this.step;
        }
    }
}
