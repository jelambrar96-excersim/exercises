...
def has_win(board, symbol):
    ...
    # check diagonal 1
    diagonal = "".join(board[row_index][col_index] for row_index,col_index in zip(range(3), range(3)))
    if dummy_row == diagonal: return True
    
    # cehckdiagonal 2
    diagonal = "".join(board[row_index][col_index] for row_index,col_index in zip(range(2, -1, -1), range(3)))
    if dummy_row == diagonal: return True
